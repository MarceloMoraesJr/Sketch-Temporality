...
config_path = Path("./configs/decoder_studies/base/")
    ...
    for output_normalization in [("rel", True), ("abs", False)]:
        ...
        for decoder in decoders:
            ...
            for seed in [42, 1999, 5342]:
                ...
                filename = f"{input_normalization[0]}_{output_normalization[0]}_decoder_{i}_seed_{seed}"
                path = config_path.joinpath(f"{filename}.yaml")
                
                config['paths']['checkpoint'] = f"./checkpoints/decoder_studies/base/{filename}"
                config['paths']['results'] = f"./results/decoder_studies/base/{filename}.pkl"
