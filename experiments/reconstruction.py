...
sketchformer = Sketchformer(
    ...
    decoder_config=DecoderConfig(
        autoregressive=config['architecture']['decoder']['autoregressive'],
        cross_attn=config['architecture']['decoder']['cross_attn'],
        condition_first=config['architecture']['decoder']['condition_first'],
        modifications_config=ModificationsConfig(
            condition_every=config['architecture']['decoder']['condition_every'],
            replace_cross_attn=config['architecture']['decoder']['replace_cross_attn'],
            remove_self_attn=config['architecture']['decoder']['remove_self_attn'],
            replace_self_attn=config['architecture']['decoder']['replace_self_attn']
        )
    ),
...
input_handler = InputHandler(input_relative_coords=config['pe']['input_relative_coords'], output_relative_coords=config['pe']['output_relative_coords'], autoencoder=True, autoregressive=config['architecture']['decoder']['autoregressive'], denoising=config['denoising'], noise_std=config['noise_std'], mask_rate=config['mask_rate'])
